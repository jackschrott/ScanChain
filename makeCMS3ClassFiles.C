...
void makeSrcFile(const string& Classname, const string& nameSpace, const string& branchNamesFile, const string& treeName, const string& objName) {
  ...
  codef << "#include \"TCanvas.h\"" << endl;
  codef << "#include \"TCut.h\"" << endl;
  codef << "#include \"TLegend.h\"" << endl;
  codef << "#include \"TStyle.h\"" << endl;
  codef << "#include \"TPaveStats.h\"" << endl;
  codef << "" << endl;
  ...
  codef << "" << endl;
  codef << "  // Histograms" << endl;
  codef << "  TDirectory *rootdir = gDirectory->GetDirectory(\"Rint:\");" << endl;
  codef << "  TCanvas *c_1 = new TCanvas(\"c_1\", \"Plots\", 200, 200, 800, 700);" << endl;
  codef << "  c_1->Divide(2,2);" << endl;
  codef << "" << endl;
  codef << "  TH1F *ht = new TH1F(\"ht\", \"ht\", 100,0,3000);" << endl;
  codef << "  TH1F *met_pt = new TH1F(\"met_pt\", \"met_pt\", 100, 0, 2500);" << endl;
  codef << "  TH1F *mt2 = new TH1F(\"mt2\", \"mt2\", 100, 0, 1500);" << endl;
  codef << "  TH1F *nJet30 = new TH1F(\"nJet30\", \"nJet30\", 11, 0, 11);" << endl;
  codef << "" << endl;
  codef << "  TH1F *ht_cut = new TH1F(\"ht_cut\", \"ht\", 100,0,3000);" << endl;
  codef << "  TH1F *met_pt_cut = new TH1F(\"met_pt_cut\", \"met_pt\", 100, 0, 2500);" << endl;
  codef << "  TH1F *mt2_cut = new TH1F(\"mt2_cut\", \"mt2\", 100, 0, 1500);" << endl;
  codef << "  TH1F *nJet30_cut = new TH1F(\"nJet30_cut\", \"nJet30\", 11, 0, 11);" << endl;
  codef << "" << endl;
  ...
  codef << "      ht->Fill(cms3.ht());" << endl;
  codef << "      met_pt->Fill(cms3.met_pt());" << endl;
  codef << "      double mt2_tmp = cms3.mt2();" << endl;
  codef << "      if(!std::isnan(mt2_tmp)) mt2->Fill(mt2_tmp);" << endl;
  codef << "      nJet30->Fill(cms3.nJet30());" << endl;
  codef << "" << endl;
  codef << "      // Applying cuts to data" << endl;           
  codef << "      if((cms3.nMuons10()==0 && cms3.nElectrons10()==0 && cms3.nPFLep5LowMT() == 0 && cms3.nPFHad10LowMT() == 0) && // lepveto cut" << endl;
  codef << "        (cms3.HLT_PFHT900() || cms3.HLT_PFJet450() || cms3.HLT_PFHT300_PFMET110() || cms3.HLT_PFMET120_PFMHT120() || cms3.HLT_PFMETNoMu120_PFMHTNoMu120()) && // trigs cut" << endl;
  codef << "        ((cms3.nVert() > 0) && (cms3.nJet30() > 1) && (cms3.deltaPhiMin() > 0.3) && (cms3.diffMetMht()/cms3.met_pt() < 0.5) && (cms3.mt2() > 200)" << endl;
  codef << "          && ( (cms3.ht() > 250 && cms3.met_pt() > 250) || (cms3.ht() > 1000 && cms3.met_pt() > 30) )) && // srbase cut" << endl;
  codef << "        (cms3.Flag_eeBadScFilter() && cms3.Flag_HBHENoiseFilter() && cms3.Flag_HBHENoiseIsoFilter() && cms3.Flag_globalTightHalo2016Filter()" << endl;
  codef << "          && cms3.Flag_badMuonFilter() && cms3.Flag_goodVertices() && cms3.Flag_EcalDeadCellTriggerPrimitiveFilter() && cms3.Flag_badChargedHadronFilter()" << endl;
  codef << "          && cms3.nJet30FailId() == 0 && cms3.met_miniaodPt() / cms3.met_caloPt() < 5.0 && cms3.nJet200MuFrac50DphiMet() == 0)) { // filters cut" << endl;
  codef << "      // Fill the following Histograms" << endl; 
  codef << "      ht_cut->Fill(cms3.ht());" << endl;
  codef << "      met_pt_cut->Fill(cms3.met_pt());" << endl;
  codef << "      if(!std::isnan(mt2_tmp)) mt2_cut->Fill(mt2_tmp);" << endl;
  codef << "      nJet30_cut->Fill(cms3.nJet30());" << endl;
  codef << "      }" << endl;
  codef << "    }" << endl;
  codef << "     // Normalizing Background" << endl;                                                                                                                                              
  codef << "    Double_t ht_norm = 1.0/(ht->Integral());" << endl;
  codef << "    ht->Scale(ht_norm);" << endl;
  codef << "    Double_t met_pt_norm = 1.0/(met_pt->Integral());" << endl;
  codef << "    met_pt->Scale(met_pt_norm);" << endl;
  codef << "    Double_t mt2_norm = 1.0/(mt2->Integral());" << endl;
  codef << "    mt2->Scale(mt2_norm);" << endl;
  codef << "    Double_t nJet30_norm = 1.0/(nJet30->Integral());" << endl;
  codef << "    nJet30->Scale(nJet30_norm);" << endl;
  codef << "" << endl;
  codef << "    Double_t ht_cut_norm = 1.0/(ht_cut->Integral());" << endl;
  codef << "    ht_cut->Scale(ht_cut_norm);" << endl;
  codef << "    Double_t met_pt_cut_norm = 1.0/(met_pt_cut->Integral());" << endl;
  codef << "    met_pt_cut->Scale(met_pt_cut_norm);" << endl;
  codef << "    Double_t mt2_cut_norm = 1.0/(mt2_cut->Integral());" << endl;
  codef << "    mt2_cut->Scale(mt2_cut_norm);" << endl;
  codef << "    Double_t nJet30_cut_norm = 1.0/(nJet30_cut->Integral());" << endl;
  codef << "    nJet30_cut->Scale(nJet30_cut_norm);" << endl;
  codef << "" << endl; 
  codef << "    // Changing appearance" << endl;                                                                                                                                                         
    codef << "    ht->SetLineColor(kBlue-7);" << endl;
  codef << "    met_pt->SetLineColor(kBlue-7);" << endl;
  codef << "    mt2->SetLineColor(kBlue-7);" << endl;
  codef << "    nJet30->SetLineColor(kBlue-7);" << endl;
  codef << "" << endl;
  codef << "    ht_cut->SetLineColor(kRed-7);" << endl;
  codef << "    met_pt_cut->SetLineColor(kRed-7);" << endl;
  codef << "    mt2_cut->SetLineColor(kRed-7);" << endl;
  codef << "    nJet30_cut->SetLineColor(kRed-7);" << endl;
  codef << "  " << endl;
  codef << "    // Clean Up" << endl;
  ...
  codef << "  // Setting legend" << endl;
  codef << "  TLegend *legend1 = new TLegend(0.68, 0.8, 0.98, 0.95);" << endl;
  codef << "  legend1->SetHeader(\"Legend\",\"C\"); // option C allows to center the header" << endl;                                                                                                               codef << "    " << endl;
  codef << "  legend1->AddEntry(ht,\"Before Selection\", \"f\");" << endl;
  codef << "  legend1->AddEntry(ht_cut,\"After Selection\", \"f\");" << endl;
  codef << "  " << endl;
  codef << "  TLegend *legend2 = new TLegend(0.68, 0.8, 0.98, 0.95);" << endl;
  codef << "  legend2->SetHeader(\"Legend\",\"C\"); // option C allows to center the header" << endl;                                                                                                               codef << "    " << endl;
  codef << "  legend2->AddEntry(met_pt,\"Before Selection\",\"f\");" << endl;
  codef << "  legend2->AddEntry(met_pt_cut,\"After Selection\",\"f\");" << endl;
  codef << "  " << endl;
  codef << "  TLegend *legend3 = new TLegend(0.68, 0.8, 0.98, 0.95);" << endl;
  codef << "  legend3->SetHeader(\"Legend\",\"C\"); // option \"C\" allows to center the header" << endl;                                                               
  codef << "  legend3->AddEntry(mt2,\"Before Selection\",\"f\");" << endl;
  codef << "  legend3->AddEntry(mt2_cut,\"After Selection\",\"f\");" << endl;
  codef << "  " << endl;
  codef << "  TLegend *legend4 = new TLegend(0.68, 0.8, 0.98, 0.95);" << endl;
  codef << "  legend4->SetHeader(\"Legend\",\"C\"); // option \"C\" allows to center the header" << endl;                                                                         
  codef << "  legend4->AddEntry(nJet30,\"Before Selection\",\"f\");" << endl;
  codef << "  legend4->AddEntry(nJet30_cut,\"After Selection\",\"f\");" << endl;
  codef << "  " << endl;
  codef << "  // Drawing Histograms" << endl;
  codef << "  c_1->cd(1); // Go to panel 1" << endl;                                                                                                                                                           
  codef << "  ht_cut->Draw(\"hist\");" << endl;
  codef << "  gPad->Update();" << endl;
  codef << "  TPaveStats *ps = (TPaveStats*)ht_cut->FindObject(\"stats\");" << endl;
  codef << "  ps->SetY1NDC(0.62); ps->SetY2NDC(0.78); // Move stats panel" << endl;                                                                                
  codef << "" << endl;
  codef << "  ht->Draw(\"hist sames\");" << endl;
  codef << "  gPad->Update();" << endl;
  codef << "  ps = (TPaveStats*)ht->FindObject(\"stats\");" << endl;
  codef << "  ps->SetY1NDC(0.45); ps->SetY2NDC(0.60);" << endl;
  codef << "  legend1->Draw(\"same\");" << endl;
  codef << "" << endl;
  codef << "  c_1->cd(2); // Change to panel 2" << endl;                                                                                                                                                          codef << "  met_pt_cut->Draw(\"hist\");" << endl;
  codef << "  gPad->Update();" << endl;
  codef << "  ps = (TPaveStats*)met_pt_cut->FindObject(\"stats\");" << endl;
  codef << "  ps->SetY1NDC(0.62); ps->SetY2NDC(0.78);" << endl;
  codef << "" << endl;
  codef << "  met_pt->Draw(\"hist sames\");" << endl;
  codef << "  gPad->Update();" << endl;
  codef << "  ps = (TPaveStats*)met_pt->FindObject(\"stats\");" << endl;
  codef << "  ps->SetY1NDC(0.45); ps->SetY2NDC(0.60);" << endl;
  codef << "  legend2->Draw(\"same\");" << endl;
  codef << "" << endl;
  codef << "  c_1->cd(3); // Change to panel 3" << endl;                                                                                                                                               
  codef << "  mt2_cut->Draw(\"hist\");" << endl;
  codef << "  gPad->Update();" << endl;
  codef << "  ps = (TPaveStats*)mt2_cut->FindObject(\"stats\");" << endl;
  codef << "  ps->SetY1NDC(0.62); ps->SetY2NDC(0.78);" << endl;
  codef << "" << endl;
  codef << "  mt2->Draw(\"hist sames\");  legend3->Draw(\"same\");" << endl;
  codef << "  gPad->Update();" << endl;
  codef << "  ps = (TPaveStats*)mt2->FindObject(\"stats\");" << endl;
  codef << "  ps->SetY1NDC(0.45); ps->SetY2NDC(0.60);" << endl;
  codef << "  legend3->Draw(\"same\");" << endl;
  codef << "" << endl;
  codef << "  c_1->cd(4); // Change to panel 4" << endl;                                                                                                                                                          codef << "  nJet30_cut->Draw(\"hist\");" << endl;
  codef << "  gPad->Update();" << endl;
  codef << "  ps = (TPaveStats*)nJet30_cut->FindObject(\"stats\");" << endl;
  codef << "  ps->SetY1NDC(0.62); ps->SetY2NDC(0.78);" << endl;
  codef << "" << endl;
  codef << "  nJet30->Draw(\"hist sames\");" << endl;
  codef << "  gPad->Update();" << endl;
  codef << "  ps = (TPaveStats*)nJet30->FindObject(\"stats\");" << endl;
  codef << "  ps->SetY1NDC(0.45); ps->SetY2NDC(0.60);" << endl;
  codef << "  legend4->Draw(\"same\");" << endl;
  codef << "  " << endl;
  codef << "  // return" << endl;
